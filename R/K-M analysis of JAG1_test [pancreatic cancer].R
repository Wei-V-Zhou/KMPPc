...
setwd('F:/博一下生活文件/学生事务/朋友/haitian')
# load packages
pkgs <- c("cgdsr", "ggplot2", "stringr", "ggpubr", "survival", 
          "survminer", "readxl","ggstatsplot", "export", "scales")
...
mycancerstudy <- getCancerStudies(mycgds)[190, 1]
research_doi <- str_extract(cancerstudy[189, 2], "\\(.+?\\)")
research_doi <- gsub("[(.*)]", "", research_doi)
...
Notch_genes <- c(
  "JAG1",    "NOTCH3",	"NOTCH2",  "APH1A",	  "HES1",
  "FZD1",	   "PSEN2",	  "FZD7",    "DTX1",    "NOTCH4",
  "DLL1",    "FZD5",	  "NOTCH1",  "PSENEN",  "JAG2",	
  "WNT5A", 	 "CUL1",	  "WNT2",    "HEYL",    "FBXW11",	
  "TCF7L2",  "ARRB1", 	"LFNG",    "PRKCA",	  "ST3GAL6")
exprSet = getProfileData(mycgds, Notch_genes, mygeneticprofile, mycaselist)
# get clinical data for the case list
myclinicaldata <- getClinicalData(mycgds, mycaselist)
# save the data
filename <- paste0(research_doi, "_survival_inputdata.Rdata")
save(exprSet, myclinicaldata, research_doi, file = filename)
...
# load data
load("TCGA, Firehose Legacy_survival_inputdata.Rdata")
...
# # DOD: Died of Disease; DOC: Died of Other Cause; AWD: Alive With Disease; NED: Not Evidence Disease
# choose_clinicaldata[which(choose_clinicaldata[ , 2] == "DOD") , 2] <- "Recurred/Progressed"
# choose_clinicaldata[which(choose_clinicaldata[ , 2] != "Recurred/Progressed") , 2] <- "Pancreatic_DiseaseFree"
# choose_clinicaldata[ , 1] = choose_clinicaldata[ , 1]/30
# colnames(choose_clinicaldata) <- c("DFS_MONTHS", "DFS_STATUS")
dat1 <- choose_clinicaldata[!is.na(choose_clinicaldata$DFS_MONTHS), ]
dat2 <- cbind(dat1, exprSet[rownames(dat1), ])
# write.csv(dat2, "Notch_TCGA_expr_pancreas.csv")
# expressed genes plot
p <- ggboxplot(dat2, x="DFS_STATUS", y="JAG1", color="DFS_STATUS", palette="jco", add="jitter")
p + stat_compare_means(method = "t.test")

gene <- c(); gene_group <- c()
n <- ncol(dat2)
# choose specific gene: JAG1
gene <- colnames(dat2)[14]
gene_group <- paste0(gene, "_group")
genes = dat2[ , 14]
gene_groups = ifelse(genes > median(genes), 'high', 'low')
dat2$genes <- genes; dat2$gene_groups <- gene_groups 
# dat2$JAG2_group = ifelse(dat2$JAG2 > quantile(dat2$JAG2)[4], 'high', 'low')
x11()
p <- ggbetweenstats(data = dat2,
                    x = gene_groups,
                    y = genes,
...
attach(dat2)
{
  ...
  kmfit <- survfit(my.surv~gene_groups, data = dat2)
  names(kmfit$strata) <- c("JAG1 high", "JAG1 low")
  ...
  # kmfit$surv <- 100 * kmfit$surv
  # plot(kmfit, col = c("red", "blue"))
  # annotate("text", x = 40, y = 0.1, parse = TRUE, size = 6, label = gene_high) +
  # annotate("text", x = 10, y = 1.0, parse = TRUE, size = 6, label = gene_low)
  ...
  p <- ggsurvplot(kmfit, palette = c("red","blue"),
                  ...
                  title = paste0("K-M plot of JAG1: ", research_doi),
